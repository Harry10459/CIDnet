...
class Msd(data.Dataset):
    def __init__(self, root, partition='train', category='msd'):
        ...
        if category == 'msd':
            if self.partition == 'train':
                data_file = r'/public/home/zhangzhong02/zpc/proto_5s/proto_DPGN5/dataset/base.json'
                with open(data_file, 'r') as f:
                    meta = json.load(f)
            if self.partition == 'val':
                data_file = r'/public/home/zhangzhong02/zpc/proto_5s/proto_DPGN5/dataset/novel.json'
                with open(data_file, 'r') as f:
                    meta = json.load(f)
            if self.partition == 'test':
                data_file = r'/public/home/zhangzhong02/zpc/proto_5s/proto_DPGN5/dataset/val.json'
                with open(data_file, 'r') as f:
                    meta = json.load(f)

            data = meta['image_names']
            label = meta['image_labels']
            lb = -1
            self.wnids = meta['label_names']
            # for l in lines:
            #     context = l.split(',')
            #     name = context[0]
            #     wnid = context[1]
