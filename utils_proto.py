...
def label2edge(label, device):
    ...
    # reshape
    # label本来是一个n*num_samples的向量，unsqueeze在倒数第一代的位置增加一个维度，repeat对应维度乘以对应数字
    # https://blog.csdn.net/HUSTHY/article/details/101649012
    label_i = label.unsqueeze(-1).repeat(1, 1, num_samples)
    # https://www.freesion.com/article/3577252288/
    label_j = label_i.transpose(1, 2)
    # compute edge
    # 这个好像是计算了所有样本之间的关系，包括query之间的（debug验证下）
...
def initialize_nodes_edges(batch, num_supports, tensors, batch_size, num_queries, num_ways, device):
    ...
    # support之间的分布关系，相同为1，不同为0。不过5w1s之间不相交，所以没有关系
    node_gd_init_support = label2edge(support_label, device)
    # query与所有support之间的概率都是相等的，即为1/NK
    ...
    # 维度=1，两个向量按行进行拼接，横向为不同的数据，竖向为该数据的向量信息
