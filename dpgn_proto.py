...
class DPGN(nn.Module):
    def __init__(self, num_generations, dropout, num_support_sample, num_sample, loss_indicator, point_metric,
        ...
        P_Sim = PointSimilarity(self.emb_size, self.emb_size, dropout=self.dropout)
        self.add_module('initial_edge', P_Sim)
        for l in range(self.generation):
            D2P = D2PAgg(self.emb_size * 2, self.emb_size, dropout=self.dropout if l < self.generation - 1 else 0.0)
            P2D = P2DAgg(2 * num_support_sample, num_support_sample)
            P_Sim = PointSimilarity(self.emb_size, self.emb_size,
    ...
    def forward(self, middle_node, point_node, distribution_node, distribution_edge, point_edge):
        ...
        for l in range(self.generation):
            ...
            if self.num_support_sample == 30:
                for i in range(0, 10, 2):
                    point_node[:, i * 3 + 5, ] = point_node[:, (i * 3):(i * 3 + 5), ].mean(1)
            if self.num_support_sample == 10:
                for i in range(0, 5):
                    point_node[:, i * 2 + 1, ] = point_node[:, i * 2, ]
            point_nodes.append(point_node * self.loss_indicator[0])
        ...
        return point_nodes, point_similarities, node_similarities_l2, distribution_similarities
