...
class DPGNTrainer(object):
    ...
    def train(self):
        ...
        :return: None
        """
        # 初始化时多一个shot给proto留位置，把proto当作一个support
        num_supports, num_samples, query_edge_mask, evaluation_mask = \
            preprocessing(self.train_opt['num_ways'],
                          ...
                          self.arg.device)
        # 此过程读取程序设定抽样值。代码设置的query数为1，在def main()部分
        ...
        for iteration, batch in enumerate(self.data_loader['train']()):
            ...
            # run the DPGN model
            point_nodes, point_similarity, node_similarity_l2, distribution_similarities = self.gnn_module(
                second_last_layer_data,
                last_layer_data,
                node_feature_gd,
                edge_feature_gd,
                edge_feature_gp)
    ...
    def eval(self, partition='test', log_flag=True):
        ...
        # 初始化时多一个shot给proto留位置，把proto当作一个support
        num_supports, num_samples, query_edge_mask, evaluation_mask = \
            preprocessing(self.eval_opt['num_ways'],
                          self.eval_opt['num_shots'] + 1,
        ...
        for current_iteration, batch in enumerate(self.data_loader[partition]()):
            ...
            # 由提供的support计算出聚类中心，并加入到标记的特征中。
            if self.train_opt['num_shots'] == 5:
                for i in range(0, 10, 2):
                    last_layer_data[:, i * 3 + 5, ] = last_layer_data[:, (i * 3):(i * 3 + 5), ].mean(1)
                    second_last_layer_data[:, i * 3 + 5, ] = second_last_layer_data[:, (i * 3):(i * 3 + 5), ].mean(1)
            if self.train_opt['num_shots'] == 1:
                for i in range(0, 5):
                    last_layer_data[:, i * 2 + 1, ] = last_layer_data[:, i * 2, ]
                    second_last_layer_data[:, i * 2 + 1, ] = second_last_layer_data[:, i * 2, ]

            # run the DPGN model
            point_nodes, point_similarity, _, _ = self.gnn_module(second_last_layer_data,
                                                                  last_layer_data,
                                                                  node_feature_gd,
                                                                  edge_feature_gd,
                                                                  edge_feature_gp)
    ...
    def compute_train_loss_pred(self,
        ...
        #     batchsize = point_node.size(0)
        #     mus = torch.zeros(batchsize, 5, 128)
        #     mus = mus.cuda()
        #     j = 0
        #     for i in range(0, 10, 2):
        #         mus[:, j, ] = point_node[:, (i * 3):(i * 3 + 5), ].mean(1)
        #         j += 1
        #     cos = nn.CosineSimilarity(dim=1, eps=1e-6)
        #     query_test = point_node[:, 30:35, ]
        #     query_node_pred = torch.zeros(batchsize, 5, 5)
        #     query_node_pred = query_node_pred.cuda()
        #     for j in range(5):
        #         for i in range(5):
        #             query_node_pred[:, j, i] = cos(mus[:, i, ], query_test[:, j, ])
        #     query_node_pred_generations.append(query_node_pred)
    ...
    def compute_eval_loss_pred(self,
        ...
        #         mus[:, j, ] = point_node[:, (i * 3):(i * 3 + 5), ].mean(1)
        #         j += 1
        #     cos = nn.CosineSimilarity(dim=1, eps=1e-50)
        #     query_test = point_node[:, 30:35, ]
        #     mus = mus.to(torch.float64)
        #     query_test = query_test.to(torch.float64)
        #     query_node_pred = torch.zeros(batchsize, 5, 5)
        #     query_node_pred = query_node_pred.cuda()
        #     for j in range(5):
        #         for i in range(5):
        ...
        #         j += 1
        #     cos = nn.CosineSimilarity(dim=1, eps=1e-6)
        #     query_test = point_node[:, num_supports:, ]
        #     query_node_pred = torch.zeros(batchsize, 5, 5)
        #     query_node_pred = query_node_pred.cuda()
        #     for j in range(5):
        #         for i in range(5):
        ...
        # prediction，节点结果的预测值，通过query与support之间的相似度比较
...
def main():
    ...
    args_opt.exp_name = 'gen{}_proto_{}way_{}shot_{}_{}'.format(config['num_generation'],
                                                                train_opt['num_ways'],
                                                                train_opt['num_shots'],
                                                                config['backbone'],
                                                                config['dataset_name'])
    ...
    elif config['backbone'] == 'WideResNet28_10':
        enc_module = WideResNet(depth=28, widen_factor=10, num_classes=200, loss_type='dist', per_img_std=False,
                                stride=1)
        print('Backbone: WideResNet28_10')
    else:
    ...
    gnn_module = DPGN(config['num_generation'],
                      train_opt['dropout'],
                      train_opt['num_ways'] * (train_opt['num_shots'] + 1),
                      train_opt['num_ways'] * (train_opt['num_shots'] + 1) + train_opt['num_ways'] * train_opt['num_queries'],
